...
export const deleteMember = async (req, res) => {
  try {
    ...
    // ❌ Block delete if NOT expired
    if (expiry >= today) {
      return res.status(400).json({
        message: "Only expired members can be deleted",
      });
    }
    ...
    res.status(200).json({
      message: "Expired member deleted successfully",
    });
...
export const permanentlyDeleteMember = async (req, res) => {
  try {
    const { id } = req.params;
